# Pending Queue Limit per Topic — Design Spec
...
Onay bekleyen haberlerde, her kategori için bekleyen haber sayısını **configurable limit** (varsayılan 50) ile sınırla; yeni pending haber geldiğinde ve periyodik temizlikte en eski pending haberleri **fetched_at** sırasına göre sil.

## Background

Şu an pending haber kuyruğu sınırsız büyüyor; bazı kategorilerde yüzlerce pending haber birikiyor (örn. 686). Bu durum gereksiz depolama maliyeti ve yönetim zorluğu yaratıyor. Mevcut akışta pending haberler eklenirken herhangi bir kırpma (trim) yapılmıyor ve periyodik temizleme yok.
...
**Out of scope:**
- Onaylı haberler için limit/retention politikası — bu değişiklik yalnızca pending içindir.
- Frontend UI değişiklikleri (liste, sayaç, uyarı vb.) — mevcut UI aynı kalır.
- Soft delete / arşivleme — yalnızca hard delete uygulanır.

## Constraints

- Eski/yeniyi belirleme kriteri **fetched_at** olacak (en eski fetched_at silinir).
- Pending limit logiği usecase → repository katmanlarında uygulanır; handler/route değişikliği gerekmez.
